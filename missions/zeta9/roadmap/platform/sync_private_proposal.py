"""Synchronize a private Prove2me *draft*; never launches a mission.

The only credential source is the gitignored workspace credentials.json. Token
material is kept out of logs and is sent only to https://prove2.me/api/v1.
"""
...
def get_token() -> str:
    ...
    if (credentials.get("access_token") and
        ...
        if credentials.get("version") != EXPECTED_VERSION:
            raise RuntimeError("Platform version differs from reviewed 0.10.9 docs")
...
def draft_body(item: dict[str, Any]) -> dict[str, Any]:
    ...
    if not readback:
        raise ValueError(f"Missing independent readback: {path.name}")
    return {
        "kind": "theorem",
...
def save_receipt(proposal: dict[str, Any], item_ids: dict[str, str],
    ...
    result = {
        ...
        "is_live_private_mission": proposal.get("status") == "Private",
    }
...
def sync() -> None:
    token = get_token()
    list_response = request("GET", "/mission-proposals?limit=100&offset=0", token)
    matches = [p for p in list_response.get("proposals", [])
    ...
    if matches:
        ...
        print(f"Created private proposal draft {proposal_id}")
    ...
    if proposal.get("visibility") != "private":
        raise RuntimeError("Refusing to synchronize a non-private proposal")

    item_ids: dict[str, str] = {}
    existing = proposal.get("items", [])
    for item in SPEC["reference_items"]:
        found = next((x for x in existing if x.get("theorem_id") == item["theorem_id"]), None)
        if found is None:
            found = request("POST", f"/mission-proposals/{proposal_id}/items", token,
                            {"kind": "reference", "theorem_id": item["theorem_id"]})
            print(f"Added reference {item['theorem_name']}")
    ...
    if (proposal.get("main_item_id") != goal_id or
        ...
        print("Set the goal, item order, fields, and private visibility")
    ...
    for item in SPEC["reference_items"]:
        remote = remote_items.get(item_ids[item["local_id"]])
        if remote is None or remote.get("theorem_id") != item["theorem_id"]:
            raise RuntimeError(f"Reference item read-back mismatch: {item['theorem_name']}")
    expected_milestones = {
    ...
    if (proposal.get("status") != "Draft" or proposal.get("visibility") != "private" or
        ...
        raise RuntimeError("Read-back verification of the private proposal failed")
    save_receipt(proposal, item_ids, milestone_rows)
    print(f"Verified private Draft: {proposal_id}; {len(item_ids)} items, "
          f"{len(milestone_rows)} milestones. Human self-audit is required to launch.")
...
def refresh_status() -> None:
    ...
    proposal = detail(token, proposal_id)
    if proposal.get("visibility") != "private":
        raise RuntimeError("Expected a private proposal")
    milestones = request("GET", f"/mission-proposals/{proposal_id}/milestones", token)
    item_ids = receipt["local_item_ids"]
    if set(item_ids.values()) != {item.get("id") for item in proposal.get("items", [])}:
        raise RuntimeError("Proposal items changed since the local draft")
    save_receipt(proposal, item_ids, milestones.get("milestones", []))
    print(f"Read back proposal {proposal_id}: status={proposal.get('status')}, "
          f"mission_id={proposal.get('mission_id')}, visibility=private")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--sync", action="store_true", help="write private draft to Prove2me")
    parser.add_argument("--refresh-status", action="store_true", help="read remote status into local receipt")
    args = parser.parse_args()
    if args.sync and args.refresh_status:
        parser.error("Choose only one mode")
    for item in SPEC["draft_items"]:
        draft_body(item)
    if SPEC["visibility"] != "private" or SPEC["main_local_id"] != "Z9.R":
        raise ValueError("Unexpected proposal target or visibility")
    if args.sync:
        sync()
    elif args.refresh_status:
        refresh_status()
    else:
        print(f"Local proposal valid: {len(SPEC['draft_items'])} draft theorems, "
              f"{len(SPEC['reference_items'])} reference, "
              f"{len(SPEC['milestones'])} milestones; no network request")
