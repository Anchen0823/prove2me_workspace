...
pre=root/'verification/family_rational_quintic_run'
def val(c,x):
    v=0
    for a in c: v=v*x+a
    assert v%4==0
...
if summary['termination_reason']=='finite_family_exhausted':
    ...
    expected=1+16*R+24*R*R
    assert summary['completed_shapes']==summary['shapes']==expected
source=root/'scripts/family_rational_quintic.cpp'
out={'search':summary,'independent_trial_division':rows,'source_sha256':hashlib.sha256(source.read_bytes()).hexdigest(),
 ...
 'scope':'Base numerator [1,2,-345,1758,70000,0]/4, add 840*t to one or two numerator coefficients indexed 1..4, t in [-50,50]; integer constant [-498960,508199]. Shapes sampled without repeat; timeout can interrupt a shape.',
...
(root/'verification/family_rational_quintic_verified.json').write_text(json.dumps(out,indent=2),encoding='utf-8')
