This proof establishes the complementary Fourier correlation bound
...
the Section 8 trapezoid sampled at n/x. The proof has no open theorem
dependencies. It combines discrete Fourier decay with the elementary
Chebyshev upper bound already proved in Mathlib.
...
Mathlib's `Chebyshev.psi_le_const_mul_self`, together with log(4) at most 2, supplies

$$\sum_{n=0}^{x}\Lambda(n)\le(\log4+4)x\le6x.$$

It follows that the correlation norm is at most 6x/1000. Finally,
...
is not needed for this stronger absolute estimate. All estimates used
here are proved in the submitted file or in Mathlib.
